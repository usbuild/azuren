...
Chat.appendChat = function (roomId, obj) {
    ...
    if (w) {
        var log = $(getChatWindow(roomId).contentArea).find(".chat-box-log").append(
...
var addUserToList = function (roomId, user) {
    var list = $(getChatWindow(roomId).contentArea).find(".chat-box-user-list ul");
...
Chat.chatHub.client.userOffline = function (groupId, user) {
    $(getChatWindow(groupId).contentArea).find(".chat-box-user-list [data-id=" + user.id + "]").remove();
};
...
Azuren.app.install("0001", "IM", "/Images/icons/chat.png", function (e) {
    ...
    if (win.isNew) {
        $(win.contentArea).html('<img src="/Images/loading.gif" style="width: 220px;height:20px; margin:60% auto 0 auto";display:block; />');
        $.get("/Chat/Index", {}, function (e) {
            $(win.contentArea).html(e);
            Chat.myId = $("#logged-id").val();
        });

        $(win.contentArea).on("click", ".chat-info-setting", function (e) {
            $.get("/Account/Settings", {}, function (e) {
                var set_win = Azuren.showWindow(600, 690, "chat-setting", "Settings", e);
                if (set_win.isNew) {
                    
                    var fun = function (e) {
                        $(set_win.contentArea).html(e);
                        if ($(set_win.contentArea).find(".error-msg").length == 0) {
                            
                            var myName = $(set_win.contentArea).find(".displayname").val();
                            
                            Chat.userInfoList[Chat.myId] && (Chat.userInfoList[Chat.myId].displayname = myName);
                            $(".my-id").html(myName);
                            $(win.contentArea).find(".username").html(myName);
                            set_win.close();
                        } else {
                            $(set_win.contentArea).find("form").ajaxForm(fun);
                        }
                    };
                    $(set_win.contentArea).find("form").ajaxForm(fun);
                }
        ...
        $(win.contentArea).on("click", ".room-list-name a", function (data) {
            data.preventDefault();
            var id = $(this).data("id");
        ...
        $(win.contentArea).on("click", ".group-remove", function (e) {
            e.preventDefault();
            var id = $(this).data("id");
        ...
        $(win.contentArea).on("click", ".add-new-room-btn", function (e) {
            e.preventDefault();
